 ...
 * @since       3.0
 */
class DemoApiModelDemoApi extends DRModel
{
    /**
     * JHttp
     *
    ...
    public function __construct($config = array())
    {
        ...
        $this->cache        = JCache::getInstance('output',array('defaultgroup' => 'com_demoapi'));

        // Set api_key
        $this->setState('access_token', HelperDemoApiApi::getApiKey());
    }
    ...
    {
        $this->componentConfig = JComponentHelper::getParams('com_demoapi');
    ...
    public function save()
    {
        ...
        $session->set('demoapi',  $post_array);

        //check if any validation error exists or not
        if( !sizeof($post_array["error_msg"]) ) {
            $session->clear('demoapi');
            /* Extract parameters needed by demoapi */

            $params = demoapi_getparams();
