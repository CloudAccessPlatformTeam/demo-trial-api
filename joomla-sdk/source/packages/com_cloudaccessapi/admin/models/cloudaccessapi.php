    ...
    public function save($data)
    {
        ...
        if (is_null($api_key) && !empty($parse_url['scheme'])) {
            if (!HelperCloudaccessApiApi::createApiKey($data)) {
                ...
                $api_key = HelperCloudaccessApiApi::getApiKey();
            }
        }

        if ($api_key) {
            list($j_template_id, $w_template_id) = HelperCloudaccessApiApi::updateEmailTemplates($data);
            $data['params']['joomla_welcome_template_id'] = $j_template_id;
            $data['params']['wordpress_welcome_template_id'] = $w_template_id;
        }
