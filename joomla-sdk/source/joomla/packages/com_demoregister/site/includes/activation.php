...
class Activation
{
	...
	static function generateURL($code)
	{
		...
		return $config->get('activation_url',JFactory::getURI()->root().'/').'index.php?option=com_demoregister&task=activate&code=' . urlencode($code);
	}
	...
	static function get($params)
	{
		...
		$q = sprintf("INSERT INTO #__demoregister_activation_codes(`code`,`params`) VALUES(%s,%s);", $db->quote($code), $db->quote($strparams));
		$db->setQuery($q);
		if(!$db->query())
		{
	...
	static function use_code($code)
	{
		...
		$q = sprintf("SELECT `params` FROM #__demoregister_activation_codes WHERE `code`= %s", $db->quote($code));
		$db->setQuery($q);
		$strparams = $db->loadResult();
		if($strparams)
		{
			$params = unserialize($strparams);
			$q = sprintf("DELETE FROM #__demoregister_activation_codes WHERE `code`= %s", $db->quote($code));
#			$db->setQuery($q);
#			$db->query();
			return $params;
		}
	...
	static function delete_older_than($age = '2 WEEK')
	{
		...
		$q = sprintf("DELETE FROM #__demoregister_activation_codes WHERE `created` < DATE_SUB(NOW(), INTERVAL %s)",$age);
		$db->setQuery($q);
		$db->query();
		die(mysql_error());
	}
